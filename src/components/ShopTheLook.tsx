...
const ShopTheLook = () => {
  ...
  const getTotalPrice = (productIds: number[]) => {
    return getLookProducts(productIds).reduce((sum, p) => sum + p.price, 0);
  };
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-muted-foreground max-w-2xl mx-auto">
            ...
            Buy together and save on styling decisions.
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {lookBundles.map((look, index) => {
            const lookProducts = getLookProducts(look.productIds);
            const totalPrice = getTotalPrice(look.productIds);
                ...
                <div className="relative aspect-[4/5] overflow-hidden">
                  ...
                  <div className="absolute bottom-4 left-4 right-4 flex gap-2">
                    {lookProducts.slice(0, 4).map((product) => (
                      <div
                        key={product.id}
                        className="w-14 h-14 rounded-lg overflow-hidden border-2 border-white/30 bg-white/10 backdrop-blur-sm"
                      >
                        <img
                          src={product.image}
                          alt={product.name}
                ...
                <div className="p-6">
                  ...
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      ...
                      <p className="text-xl font-bold text-gold">₹{totalPrice.toLocaleString()}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      onClick={() => handleAddAllToCart(look)}
                    >
            ...
            <>
              ...
              <div className="grid gap-4 mt-4">
                {getLookProducts(selectedLook.productIds).map((product) => (
                  <div key={product.id} className="flex gap-4 p-4 border border-gold/20 rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-lg font-bold text-gold mt-1">₹{product.price.toLocaleString()}</p>
                    </div>
                ...
                <div className="flex items-center justify-between p-4 bg-muted/50 rounded-lg">
                  <div>
                    <p className="text-sm text-muted-foreground">Total for all {getLookProducts(selectedLook.productIds).length} pieces</p>
                    <p className="text-2xl font-bold text-gold">
                      ₹{getTotalPrice(selectedLook.productIds).toLocaleString()}
                    </p>
                  </div>
                  <Button
                    size="lg"
                    className="bg-gold hover:bg-gold/90 text-primary-foreground"
                    onClick={() => {
                      handleAddAllToCart(selectedLook);
